...
public class BoardController {
    ...
    public String list(Model model , HttpSession session){
        ...
        List<Board> boards = boardService.list(); // ← 이 줄이 필요함!
    ...
    public String editform(@PathVariable Long id, HttpSession session, Model model){
        ...
        model.addAttribute("comments", commentService.getComments(board)); // 댓글 리스트 추가
        return "board-edit";
    }
    @PostMapping("/boards/{id}/comments")
    public String addComment(@PathVariable Long id, CommentDTO dto, HttpSession session) {
        Member loginUser = (Member) session.getAttribute("loginUser");
        if (loginUser == null) return "redirect:/login";

        Board board = boardService.findById(id);
        commentService.addComment(board, loginUser, dto);
